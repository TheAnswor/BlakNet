...
---
Task ID: SOCIAL-NETWORK
Agent: specialized sub agent (general-purpose) — senior frontend engineer (BlakNet)
Task: Convert the Network CRM page (`src/views/dashboard/network.tsx`) into a social Following/Followers page (like LinkedIn connections). Replace the CRM contacts UI entirely with a social page; keep `NetworkView` exported.

## Work Log
1. Read `worklog.md` + the existing `src/views/dashboard/network.tsx` (CRM contacts UI with add/edit/delete Dialog, search, category filter) + supporting files: `src/lib/types.ts` (`Business` type with `followerCount?: number`), `src/lib/store.ts` (`useApp` → `navigate`, `route`, `authUser`), `src/lib/api.ts` (`api<T>(path)`), `src/components/blaknet/badges.tsx` (`Pill`, `VerifiedBadge`), `src/components/blaknet/section.tsx` (`EmptyState`), `src/lib/format.ts` (`formatNumber`, `provinceCity`, `timeAgo`, `initials`), `src/components/ui/tabs.tsx`, `src/components/ui/avatar.tsx`, `src/components/ui/separator.tsx`, `src/components/blaknet/business-card.tsx` and `src/views/dashboard/following.tsx` (existing card patterns to match the brand language), and `src/components/blaknet/follow-button.tsx`. Verified the two backend endpoints (`/api/businesses/followed` and `/api/businesses/owner`) exist.

2. **Backend tweak** — `src/app/api/businesses/owner/route.ts` (*edited, minimal*): the existing route did not expose `followerCount`, but the Followers tab needs it. Added `_count: { select: { reviews: true, follows: true } }` to the Prisma `include` and mapped items to `({ ...b, followerCount: b._count.follows })`. This mirrors the existing pattern used in `src/app/api/admin/businesses/route.ts` (line 47, 72) and `src/app/api/businesses/[slug]/route.ts` (line 72). Backward-compatible — only ADDS fields; the existing `MyBusinessesView` in `src/views/dashboard/businesses.tsx` consumes `b.views`, `b.profileCompletion`, `b.logoUrl`, `b.industry`, etc. — all of which are preserved by the `...b` spread.

3. **Rewrote `src/views/dashboard/network.tsx`** (*full rewrite, 470 lines*) — `"use client"` at the top, still exports `NetworkView`. Removed every CRM artifact: no more `Contact` type import, no `CONTACT_CATEGORIES`, no `Textarea`/`Label`/`Dialog`/`Select`/`Input` imports, no add/edit/delete handlers, no Contact form, no delete-confirm Dialog, no `categoryTone` helper, no search/category filter, no `ContactCard`. The old Contact backend (`/api/contacts`) is intentionally untouched and simply no longer wired into the UI.

   - **Imports**: `useEffect`/`useState` from React; `useApp` from `@/lib/store`; `api` from `@/lib/api`; shadcn `Button`, `Skeleton`, `Tabs`/`TabsList`/`TabsTrigger`/`TabsContent`; blaknet `Pill`, `VerifiedBadge`, `EmptyState`; `formatNumber` + `provinceCity` from `@/lib/format`; `Business` type from `@/lib/types`; `cn` from `@/lib/utils`; lucide icons `Users`, `UserPlus`, `Building2`, `UserCheck`, `ArrowRight`, `Sparkles`, `AlertCircle`, `MapPin`.
   - **`NetworkView`**: heading "My Network" + eyebrow + subtitle "People and businesses you follow, and those following you." Two fetch effects (parallel on mount): `GET /api/businesses/followed` → `following[]` + `followingLoading`/`followingError`; `GET /api/businesses/owner` → `owned[]` + `ownedLoading`/`ownedError`. `totalFollowers = owned.reduce((s,b) => s + (b.followerCount ?? 0), 0)`. Renders a `<Tabs value=tab onValueChange>` with two triggers (count badges appear once loaded + non-zero), each TabsContent delegates to a focused panel component.
   - **`FollowingPanel`**: loading skeleton grid (6 cards) + count skeleton; error → `EmptyState` (AlertCircle) with "Try again"; empty → `EmptyState` (Users) titled "You're not following any businesses yet." with description + action button "Explore the directory" → `navigate({ name: "directory" })`; populated → "Following X businesses" count line + grid of `FollowingCard` + a centered note: "User-to-user following is coming soon — for now, follow businesses to stay updated." (Sparkles icon, sage accent). The note appears in BOTH the empty state and populated state per the spec ("Add a note").
   - **`FollowingCard`**: clickable `<button>` with `card-lift` + `animate-fade-in-up` (staggered by `Math.min(index, 8) * 40ms`). Top strip is `bg-ink-grain h-20` with absolutely-positioned logo (img with fallback to ink-colored initials tile, both `h-14 w-14 rounded-lg border-2 border-card`), Verified badge pill in the top-right when `verificationStatus === "VERIFIED"`. Body: business name + ArrowRight (animated on hover), tagline (or "No tagline yet." muted), industry + location line (Building2/MapPin), footer "Following" pill with sage-tinted `UserCheck`. Click → `navigate({ name: "business", slug })`.
   - **`FollowersPanel`**: loading skeleton (3 cards) + count skeleton; error → `EmptyState` (AlertCircle) with Try again; empty (no owned businesses) → `EmptyState` (Building2) titled "No followers yet." with description "Create a business page to start building your following." + action button "Add a business" (UserPlus icon) → `navigate({ name: "dashboard-business-new" })`; populated → "X people follow your businesses" count line (singular/plural aware, `formatNumber`) + grid of `FollowerCard`.
   - **`FollowerCard`**: same brand-card chrome as `FollowingCard` (logo strip, Verified badge, name + ArrowRight, tagline, industry + location). Footer shows the follower count prominently — `Users` icon (sage) + "{count} followers" — and a "View profile" hint that animates to ink on hover. Click → `navigate({ name: "business", slug })`.
   - **`NetworkCardSkeleton`**: shared skeleton used by both panels (h-20 strip + 5 line skeletons).
   - **TypeScript**: no `any` anywhere. `FollowedBusiness = Business & { followedAt?: string | null }` (matches the existing pattern from `following.tsx`). Tab state typed as `NetworkTab = "following" | "followers"`.
   - **Brand discipline**: only brand tokens used (`bg-ink`, `text-cream`, `bg-cream`, `text-sage`, `bg-ink-grain`, `bg-card`, `border-border`, `text-foreground`, `text-muted-foreground`, `font-display`) + the brand utility classes (`.card-lift`, `.btn-lift`, `.animate-fade-in-up`). NO blue/indigo. NO gradients. Action buttons use `bg-ink text-cream hover:bg-ink/90` + `btn-lift`.

4. Lint: `bun run lint` → 0 errors, 1 pre-existing warning in `src/app/page.tsx` line 103 (Unused eslint-disable directive) — NOT in any file I touched. My network.tsx + the owner route are clean.

5. Dev server: confirmed healthy. `tail -20 /home/z/my-project/dev.log` ends with `✓ Compiled in 690ms` and a clean `GET /api/events 200` — no compile errors from my new file or the owner-route edit.

## Stage Summary
- **Files created**: none.
- **Files edited (rewrite)**: `src/views/dashboard/network.tsx` — full rewrite from CRM contacts UI to social Following/Followers page; still exports `NetworkView`; `"use client"` at top.
- **Files edited (backend)**: `src/app/api/businesses/owner/route.ts` — minimal additive change to expose `followerCount` (mirrors admin/businesses route pattern). Backward-compatible (only adds fields; existing `MyBusinessesView` consumer unaffected).
- **CRM backend untouched**: the `/api/contacts` routes and `Contact` Prisma model are intentionally NOT modified — they remain available; the new UI just doesn't reference them, as the spec required.
- **Lint**: 0 errors, 0 warnings in touched files (one pre-existing warning in `src/app/page.tsx` was already there before this task).
- **Dev server**: running on :3000, healthy. `GET /api/events` 200; compiles cleanly after edits.
- **Next actions for main agent**: (1) Smoke-test `#/dashboard/network` — verify the "Following" tab loads the followed-businesses grid (click a card → `#/business/<slug>`), the "Followers" tab loads the owned-businesses grid with follower counts, the tab count badges update, and both empty states render with their action buttons ("Explore the directory" → `#/directory`; "Add a business" → `#/dashboard/businesses/new`). (2) Confirm the "user-to-user following is coming soon" note renders below the grid (and in the empty state). (3) Optional follow-up — the legacy `/api/contacts` backend is now unused by the UI; consider whether to deprecate/remove it in a future cleanup pass (out of scope for this task).
